...
class Evaluator:
    ...
    def __init__(self, results_path: str, dataset_name: str, config: Optional[Config] = None):
        ...
        self.config = config
        self.enable_llm_eval = bool(config and config.evaluation.enable_llm_eval)
        self.llm_evaluator = None
        
        if self.enable_llm_eval:
            # Lazy-initialize LLMEvaluator only when cached llm_judge labels are unavailable.
            logger.info(f"Evaluator initialized with LLM evaluation (lazy) for dataset: {dataset_name}")
        else:
    ...
    async def evaluate(self) -> Dict[str, Any]:
        try:
            ...
            if self.enable_llm_eval:
                # Avoid duplicate judge API calls: query pipeline may already persist llm_judge per item.
                cached_metrics = self._calculate_metrics_from_cached_llm_judge(results)
                if cached_metrics is not None:
                    answer_metrics = cached_metrics
                    logger.info("Using cached llm_judge labels from results.json (skip second LLM judge pass)")
                else:
                    if self.llm_evaluator is None:
                        self.llm_evaluator = LLMEvaluator(self.config, str(self.results_path), self.dataset_name)
    ...
    def _to_bool_label(self, value: Any) -> Optional[bool]:
        if value is None:
            return None
    ...
    def _calculate_metrics_from_cached_llm_judge(self, results: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """Return cached LLM metrics when every answerable sample already has llm_judge label.
        If any answerable sample lacks label, return None so caller can run live LLM evaluation."""
        if not results:
        ...
        for result in results:
            ...
            label = self._to_bool_label(result.get('llm_judge'))
            if label is None:
                return None
            if label:
                correct_answers += 1
        ...
        return {
            'evaluation_method': 'llm_based_cached_labels',
            'eval_model': self.config.evaluation.eval_model if self.config else '',
