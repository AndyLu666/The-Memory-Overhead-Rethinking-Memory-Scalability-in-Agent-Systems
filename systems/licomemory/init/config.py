...
class RetrieverConfig:
    ...
    top_session_ids_limit: int = 20       # Max unique session ids stored in trace
    ...
    adaptive_entity_top_k: bool = True    # Auto-increase entity top_k on large graphs
    adaptive_entity_node_threshold_large: int = 3000
    adaptive_entity_node_threshold_huge: int = 6000
    adaptive_entity_top_k_large_graph: int = 12
    adaptive_entity_top_k_huge_graph: int = 20
    entity_hop_expansion: int = 1         # 1 means disabled; 2 means one-hop expansion
    min_candidate_triples: int = 20       # Ensure enough candidate triples before rerank
    lexical_fallback_limit: int = 200     # Extra lexical fallback triples scan budget
    triple_lexical_weight: float = 0.35   # Extra lexical match weight in triple ranking
    triple_answer_session_bonus: float = 0.12
    triple_external_session_penalty: float = 0.03
    enable_global_semantic_fallback: bool = False
    global_semantic_fallback_top_n: int = 120
    global_semantic_entity_conf_threshold: float = 0.12
    enable_direct_chunk_fallback: bool = False
    direct_chunk_top_k: int = 4
    direct_chunk_min_overlap: int = 1
    direct_chunk_min_overlap_hard: int = 1
    direct_chunk_use_time_decay: bool = True
    direct_chunk_answer_session_bonus: float = 0.25
    direct_chunk_external_session_penalty: float = 0.05
    load_only_attach_corpus_chunks: bool = False
    source_prior_hard_task_multiplier: float = 2.0
    # ReAct-style multi-hop query settings (disabled by default)
    enable_react_multihop: bool = False
    react_max_steps: int = 3
    react_max_context_triples: int = 24
    react_max_context_chunks: int = 12
    react_agent_temperature: float = 0.0
    react_agent_max_tokens: int = 512
...
class EvaluationConfig:
    ...
    eval_prompt_style: str = 'memos_json'  # Use MemOS-style CORRECT/WRONG JSON judge by default
    eval_num_runs: int = 1                # MemOS-style repeated judge runs when needed
    eval_temperature: float = 0.0         # Temperature for evaluation model
    eval_max_tokens: int = 10             # Max tokens for evaluation response
    eval_api_key: str = ""                # If empty, fallback to env or main llm key
    eval_base_url: str = ""               # If empty, fallback to env or main llm base_url
    eval_timeout: int = 0                 # If <=0, fallback to main llm timeout
...
class QueryLLMConfig:
    ...
    retry_attempts: int = 0  # If 0, will use main llm config
    retry_backoff: float = 0.0  # If <=0, will use main llm config
    retry_backoff_max: float = 0.0  # If <=0, will use main llm config
    fail_on_error: Optional[bool] = None  # If None, will use main llm config
...
class Config:
    ...
    def parse(cls, config_path: Path, dataset_name: str = None) -> 'Config':
        ...
        config_dict = _expand_env_placeholders(config_dict)
