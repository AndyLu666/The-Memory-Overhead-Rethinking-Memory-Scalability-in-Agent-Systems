...
class DynamicMemory:
    ...
    async def add_single_session(self, session_corpus: List[Dict[str, Any]]) -> None:
        ...
        # Avoid repeated query-time embedding computation for newly added content.
        try:
            await self._precompute_entity_embeddings(entities)
            await self._precompute_relationship_embeddings(relationships)
        except Exception as e:
            logger.warning(f"Failed to precompute embeddings for incremental session add: {e}")
    ...
    async def _dynamic_add_chunks(self, chunks: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to precompute embeddings for dynamic add: {e}")
    ...
    async def _precompute_entity_embeddings(self, entities: List[Dict[str, Any]]) -> None:
        ...
        # Only compute embeddings for entities that don't already have one on graph nodes.
        entity_texts = []
        entity_names = []
        ...
        for entity in entities:
            entity_name = entity.get('entity', '')
            if not entity_name or entity_name in seen_entity_names:
                continue
            seen_entity_names.add(entity_name)

            if entity_name in self.graph_builder.graph.nodes:
                node_embedding = self.graph_builder.graph.nodes[entity_name].get('embedding')
        ...
        if not entity_texts:
            logger.info(f"Entity embedding precompute skipped: all entities already embedded ({skipped_existing} skipped)")
            return
        ...
        if not embeddings or len(embeddings) != len(entity_texts):
            ...
            raise RuntimeError(
                f"Failed to get embeddings for entities (got {got}, expected {len(entity_texts)})"
            )
        ...
        logger.info(
            f"Entity embedding precompute: computed={len(entity_names)} skipped_existing={skipped_existing}"
        )
    ...
    async def _precompute_relationship_embeddings(self, relationships: List[Dict[str, Any]]) -> None:
        ...
        # Only compute embeddings for edges without existing embeddings.
        triple_texts = []
        triple_keys = []
        seen_edges = set()
        skipped_existing = 0
        ...
        for rel in relationships:
            ...
            if self.graph_builder.graph.has_edge(src, tgt):
                edge_embedding = self.graph_builder.graph.edges[src, tgt].get('embedding')
            ...
            triple_keys.append(edge_key)

        if not triple_texts:
            logger.info(f"Relationship embedding precompute skipped: all edges already embedded ({skipped_existing} skipped)")
            return
        ...
        if not embeddings or len(embeddings) != len(triple_texts):
            got = len(embeddings) if embeddings else 0
            raise RuntimeError(
                f"Failed to get embeddings for relationships (got {got}, expected {len(triple_texts)})"
            )
        ...
        logger.info(
            f"Relationship embedding precompute: computed={len(triple_keys)} skipped_existing={skipped_existing}"
        )
