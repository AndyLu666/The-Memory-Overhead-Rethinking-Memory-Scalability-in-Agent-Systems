...
class HippoRAG:
    ...
    def load_existing_openie(self, chunk_keys: List[str]) -> Tuple[List[dict], Set[str]]:
        ...
        if not self.global_config.force_openie_from_scratch and os.path.isfile(self.openie_results_path):
            try:
                ...
                logger.warning(
                    "Failed to load existing OpenIE cache from %s (%s). Rebuilding this cache from scratch.",
    ...
    def merge_openie_results(self,
        ...
        for chunk_key, row in chunks_to_save.items():
            ...
            try:
                cleaned_entities = []
                for entity in ner_results_dict[chunk_key].unique_entities:
                    if entity is None:
                        continue
                    text = str(entity).strip()
                    if text:
                        cleaned_entities.append(text)
    ...
    def save_openie_results(self, all_openie_info: List[dict]):
        ...
        for chunk in all_openie_info:
            entities = chunk.get('extracted_entities') or []
            cleaned_entities = []
        ...
        if len(all_openie_info) > 0:
            ...
            target_dir = os.path.dirname(self.openie_results_path)
            os.makedirs(target_dir, exist_ok=True)
            fd, tmp_path = tempfile.mkstemp(
                prefix=".openie_results_tmp_",
                suffix=".json",
                dir=target_dir,
            )
            try:
                with os.fdopen(fd, "w", encoding="utf-8") as f:
                    ...
                    f.flush()
                    os.fsync(f.fileno())
                os.replace(tmp_path, self.openie_results_path)
            finally:
    ...
    def graph_search_with_fact_entities(self, query: str,
        ...
        nonzero_occurs = number_of_occurs > 0
        phrase_weights[nonzero_occurs] /= number_of_occurs[nonzero_occurs]
        phrase_weights[~nonzero_occurs] = 0.0
